...
const Content = () => {
  ...
  return (
    <div className="flex-wrap lg:flex-nowrap flex gap-10">
      <motion.div variants={fadeTop} {...motionStep} className="relative min-w-[280px]">
        <div>
          {worked_data.map((e: WorkedTypes, i: number) => (
            <div key={i} onClick={() => setActiveIndex(i)} ref={(el) => (tabsRef.current[i] = el)} className={cx(activeIndex === i && "bg-sky-600/10 text-sky-400", "tab__item")}>
              {e.label}
            </div>
          ))}
        </div>

        <span className="absolute left-0 block w-1 bg-sky-400 transition-all duration-300" style={{ top: tabSidelineTop, height: tabSidelineHeight }} />
      </motion.div>
